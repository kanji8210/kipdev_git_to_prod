...
class KIPDEV_Rollback {
    ...
    public function rollback($repo_id, $version) {
        try {
            ...
            $backup_path = KIPDEV_BACKUP_DIR . 'backup_' . $repo->target_slug . '_' . $version;
            ...
            if (file_exists($target_path)) {
                $current_backup = KIPDEV_BACKUP_DIR . 'pre_rollback_' . $repo->target_slug . '_' . date('Ymd_His');
    ...
    public function get_available_backups($repo_id) {
        $backups = [];
        $files = glob(KIPDEV_BACKUP_DIR . 'backup_*');
