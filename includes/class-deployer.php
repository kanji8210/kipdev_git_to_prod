<?php
class KIPDEV_Deployer {
    private $logger;
    private $validator;
    
    public function __construct() {
        $this->logger = new KIPDEV_Logger();
        $this->validator = new KIPDEV_Validator();
    }
    
    public function deploy($repo_id, $commit_hash = null, $branch = null) {
        // Check if already deploying
        if (get_transient('kipdev_deploying_' . $repo_id)) {
            return ['success' => false, 'message' => 'Deployment already in progress'];
        }
        
        set_transient('kipdev_deploying_' . $repo_id, true, 300);
        
        try {
            $repo_manager = new KIPDEV_Repo_Manager();
            ...
            delete_transient('kipdev_deploying_' . $repo_id);
    ...
    private function clone_repository($repo, $branch) {
        $temp_dir = KIPDEV_BACKUP_DIR . 'temp_' . uniqid();
    ...
    private function deploy_files($source, $destination) {
        ...
        if (file_exists($destination)) {
            $backup_dir = KIPDEV_BACKUP_DIR . 'backup_' . basename($destination) . '_' . date('Ymd_His');
